...
| S-06 | razer input diagnostics | awaiting-verification | | 2026-09-07 | Diagnosis only, depends on S-04 (already verified). Nothing to write: the deliverable is the diagnostic command set in the handoff. Waiting on `evtest`/`libinput debug-events` output for volume, mute and brightness keys (with and without Fn), `/proc/bus/input/devices` for the internal keyboard's vendor:product id, `lsusb`, and `ls /sys/bus/iio/devices/` (closes `Q-F01`). `DONE WHEN` is just the codes being known — the `hwdb` rule itself is S-46, not this step |
