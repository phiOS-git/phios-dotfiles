...
| `wireguard-tools` | extra | VPN WireGuard, guidata da `phi vpn` e dalla sezione Connectivity. I `.conf` dei tunnel sono dell'utente, in `~/.config/phi/wireguard/` — mai nel repo. `up`/`down` via `sudo -n` (drop-in `sudoers.d/49-phi-vpn`, mai applicato). Aggiunto da `Out-of-plan: settings-overhaul` | OOP-42 |
...
| `Q-N03` | **Ridecisa (ADR 134, out-of-plan, 2026-09-11)**: ~~btop: workspace normale a ID alto e persistente, o special workspace a comparsa?~~ ~~Special workspace (ADR 122)~~ — mai funzionato sull'hardware reale; ora btop e Steam su workspace numerati normali (`10` e `9`), resi in barra con un glifo invece della cifra | shell §2 | M2 |
...
| 134 | 2026-09-11 | shell §2, `Q-N03`, inverte `ADR 122` | **Inverte ADR 122**, su richiesta esplicita dell'utente. Il modello special-workspace di btop non ha mai funzionato sull'hardware: btop non partiva dal pulsante, il toggle cambiava lo stato in barra senza rivelare nulla, e un workspace numerico e quello special potevano risultare entrambi "attivi". btop e Steam passano a **workspace numerati normali** — btop su `10` (`persistent`, avviato con la sessione da `hyprland.lua`), Steam su `9` (non persistente). La barra rende i workspace `9`/`10` con un glifo (Steam / monitor) invece della cifra, all'estremità destra della striscia (`Bar/modules/Workspaces.qml` + il nuovo `Bar/workspace-icons.json`); il modulo `specialWorkspaces` e `Bar/pinned-apps.json` sono rimossi. `btop`/`Steam su workspace dedicato` (§2) **resta chiuso** — cambia solo il tipo di workspace. Traccia `Out-of-plan: workspace-icons`, `phi` non toccato | Special workspace con toggle in barra (ADR 122); workspace a ID molto alto raggiungibile solo dall'icona | Sì |
