...
No `packages.txt` yet. The services will come later, and the `[phi]` repository
it will serve over the overlay network is built separately, so there is nothing
to declare there. It is listed in `hosts/mini.txt` so the place exists before
the first real service.

`system/` carries what is already applied on `mini` by hand: the `zram`
...
machine to match (this file has repository-side canonical text to reconcile
against).
